...
# Города РФ, Украины и Казахстана с населением >300 тыс (примерный список)
allowed_cities = [
    # Россия (пример)
    ...
    "Усть-Каменогорск", "Семей", "Костанай",
]
...
def send_city_request(chat_id):
    ...
    # Формируем кнопки с городами (покажем 15 городов по кнопкам, чтобы не засорять)
...
def city_callback(call):
    ...
    if call.data == "other_city":
        bot.send_message(chat_id, "Введите ваш город вручную (только из доступных городов РФ, Украины и Казахстана с населением более 300 тыс):")
...
def manual_city_input(message):
    ...
    if city in allowed_cities:
        ...
        bot.send_message(chat_id, "Извините, мы не работаем в вашем городе. Пожалуйста, введите город из списка.")
